...
**Last updated:** 18 August 2026 (#5 declared) · branch `praneetdhoolia/convergence-pilot-arms`
...
| Blocking state | **None.** #5 is DECLARED (§9.43) and the gate it held is open. Next: **the ride sitting** (4.2.3) |
...
| Input registry | **298 fields** (§9.43 added `RUN.relaxation.settle_margin_iterations`) — every one with units, provenance and a sweep, held-fixed rule or derived identity; ledger **0** with `--strict` gating CI; reach 69/69 |
...
| 4.2.3 | **THE RIDE SITTING — the active lane (owner direction, 18 Aug).** Mechanism inventory done: `ride` is network-routed but not a QSim main mode, so it moves at the router's estimate while car pays the queue, and it pays no parking or pickup friction. Fix, in one sitting, all values declared: **(a)** bind ride's time to car's *congested* travel times (controler binding + declared toggle); **(b)** a declared, swept pickup/drop-off time per ride leg — the short-bin residual is exactly the shape a missing fixed friction produces; **(c)** re-solve `asc_car_passenger` against the observed 0.35 passengers-per-driver (#31's constraint, #9's solver, at the declared horizon). Full passenger-in-vehicle coupling (joint plans) stays the logged backlog alternative — heavier toolchain change, revisit only if (a)–(c) leave a residual. **UNGATED** — #5 declared 18 Aug, so the solver runs at the settled horizon of **1000 iterations** (§9.43). Cost the solver runs and get owner approval before launching | #28, #31, #9 | attended 2–3 days · wall 1–2 days of runs |
