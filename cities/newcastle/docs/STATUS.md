...
| Blocking state | **The FIRST CONVERGED RUN of the all-physical model is IN PROGRESS**: `results/phys1000_25pct/` (S2 × WEEKDAY, 25% × 1000, launched 21 Aug 00:40, owner-approved, ETA ~48–58 h at the measured ~190–200 s/iteration). It re-baselines everything at once: the emergent ride share vs 20.60, walk vs 13.40 (carrying both #30's generation deficit AND §9.55's displaced ride mass), the §9.50 constrained base (C5 + report from its fit). #54's accounting fix is MERGED (PR #58), so the run's own gate is trustworthy. **Active lane while it runs: nothing that touches the machine** — close-out on completion, then the directive measurement halves |
