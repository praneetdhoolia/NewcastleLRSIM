...
**Last updated:** 18 August 2026 (#5 closed; ride pairing Tier 1 merged in PR #40; sampling unit is the household; **the plan is reprioritised against the goal and four tasks are proposed for deletion** — see [`NEXT_AGENT_BRIEF.md`](handover/NEXT_AGENT_BRIEF.md) §7) · branch `main`
...
| **4.2.5** | **Bind the escort tour to the person being escorted** — `ATTRACTION_ALIAS = {'HX': 'HE'}` and `DEPART['HX'] = DEPART['HE']` draw an escort's destination and time from the education DISTRIBUTION, never the escorted child's actual school, so household members never co-locate: **0.104% of ride trips share an OD with a household car trip**. RE-TARGET existing HX tours, never add any (the HX rate is already calibrated to `Serve passenger` 10–19.5%, OBSERVED); derive eligibility from licence + vehicle only, there being **no target for who-drives-whom**; do not force return symmetry. **This unblocks 32.7% of trips** — the coupling is built and waiting | #31 (supply half), §9.44 | attended 2–3 days |
| **4.2.6** | **The elderly employment defect** — `build_population.py` claims age-conditional labour force status (G46) and applies **one flat 15+ rate** from G43 to every adult: **65–74 at 52.2% employed, 75+ at 47.7%** against real ~15–25% and ~3–5%, i.e. **~35,000 phantom elderly commuters**, exactly the cohort that RIDES rather than drives. Also `student_status` = `full_time` for 100% of under-18s including all 22,115 aged 0–4. G46 is already in the package — a real fix with real data, not a re-parameterisation. Sequenced AFTER 4.2.5 because it contaminates its validation | population defect | attended 1 day |
