...
| What is PHYSICALLY simulated (measured 20 Aug, §9.49/§9.52/§9.53) | **7 modes physical, and paired ride passengers now ride INSIDE the car**: `car`; **`truck`** at PCE 2.0 — smoke: 913 trips / 140,380 traversals at 1%; **`motorbike`** at PCE 0.4 on the measured G62 anchor — smoke: 12 riders / 52 trips; **bus** 1,448 vehicles / PCE 2.8 / 22,102 shared links; **rail** 332; **tram** 252; **ferry** 107; **paired `ride` physically boards the driver's car (§9.53)** — probe: 67–71 boardings/iteration at 1%, 2 missed (Tier-1 fallback, counted), verified in the events. **Unpaired ride stays teleported pending #48's re-moding policy** (the owner's §4 dossier decision). Taxi/rideshare not a mode (4.4/#49 tier plan) |
...
| # | Task | Issue | State |
|---|---|---|---|
| **4.5.1** | **Physical ride** — every ride trip a passenger physically in a car, share tuned to the observed 20.60% | **#48 — FIRST** | 🟡 **mechanism BUILT and probe-verified (§9.53)**: the ×12 gap DECOMPOSED (×2.24 modes / ×6.91 window / ×2.73 links — `realisation_gap_bind1000_25pct.json`) forced dossier option C; `JointRideEngine` physically boards paired passengers (probe: 67–71/iteration, 2 missed→Tier-1 fallback, event-verified). **Open: the owner's §4 re-moding policy for the UNPAIRED majority** (household-only pairing caps physical ride at OD-coincidence 15.31% vs observed 20.60%), the window layer (joint departure times, a replanning question), and a converged-run measurement |
