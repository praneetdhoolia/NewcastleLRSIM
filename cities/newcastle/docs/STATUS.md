...
**Last updated:** 21 August 2026, 13:00 (**the first converged arm was STOPPED at owner instruction during iteration 136** — quarantined to `results/_aborted_20260821/phys1000_25pct`, no `_run.json`, relaunch needs fresh approval; 135 completed iterations of trajectory diagnostics preserved. Earlier the same day: launched 00:40 with approval after the 50-iteration shakedown. Preceded the same night by: **#54 FIXED and merged, PR #58** — accounting read from the events stream, duplicate engine aborts counted separately; **the 50-iteration 25% shakedown `phys50_25pct`** — rc=0, accounting closes on every mode, nothing broken, median 261.1 s/iteration measured; **§9.56** — the saturated single events thread diagnosed and `RUN.machine.event_handler_threads` = 4 adopted after bit-identity verification, ~21% off the wall; **500 iterations considered and rejected** on `bind1000_25pct`'s own trajectory — car still moved +2.14 pp between iterations 500 and 790, so a 500 horizon truncates search) (**every person-transport mode is now PHYSICALLY simulated, §9.49–§9.55**: freight PR #46; the §8.5 constrain-and-report decision PR #47 — its base arm owner-stopped at ~iteration 20, deliverable 5 still open; the four §9.51 owner directives PR #51 with the research dossiers and the measured gap decomposition; motorbike PR #52; physical ride boarding + walk/bike physical + the emergent ride share PR #53. **The demand family and the model changed several times today with no completed run between them — one family boundary, no comparison across it.** Active lane: **the owner's 4.5.0 sequencing decision, then the first converged run of the all-physical model** — every arm needs stated-cost approval) · the session landed on `main` via **PR #56** (the stacked PRs #46–#55 merged into their base branches; #56 carried the union). Start from `main`
...
| Blocking state | **The first converged arm was STOPPED at owner instruction during iteration 136 (21 Aug ~11:52)** and quarantined to `results/_aborted_20260821/phys1000_25pct` — no `_run.json`, not a result; relaunch requires fresh stated-cost approval (~190–200 s/iteration measured, ~2.2 days). **Awaiting the owner's further instructions.** When it relaunches, it still re-baselines everything at once: the emergent ride share vs 20.60, walk vs 13.40, the §9.50 constrained base (C5 + report). #54's accounting fix is MERGED (PR #58), so the run's own gate is trustworthy |
...
| Input registry | **327 fields**, verified against the generated contract 21 Aug (§9.56 +1 `RUN.machine.event_handler_threads`; the 20 Aug close-out stated 327 when the on-disk count was 326 — an off-by-one in the running arithmetic, corrected by recount, not by removal) — every one with units, provenance and a sweep, held-fixed rule or derived identity; ledger **0** with `--strict` gating CI |
...
- **On the ALL-PHYSICAL model (21 Aug, `phys50_25pct`):** median **261.1
  ...
  measured (`evthreads_timing`), ~21% off the wall; the converged-arm
  projection is **~48–58 h**, expected to improve as the search moves demand
  off walk. ~100 s/iteration is NOT reachable without changing the model —
  the event volume is the price of the all-physical directive.
...
| # | Task | Issue | State |
|---|---|---|---|
| **4.5.1** | **Physical ride** — every ride trip a passenger physically in a car, share tuned to the observed 20.60% | **#48 — FIRST** | 🟢 **BUILT end-to-end, probe-verified (§9.53 + §9.55)**: the ×12 gap DECOMPOSED forced option C; `JointRideEngine` boards paired passengers; **unpaired ride re-modes to physical walk — no teleportation, ride share emergent from driver supply** (probe: 2,758 re-moded at iteration 0; final ride = 67 trips, all boarded). Open: the converged-run measurement (where the displaced ~10 pp of demand settles vs observed 20.60% — the unobserved non-household-lift gap is REPORTED), the window layer (joint departure times), the summariser's stuck-attribution defect |
