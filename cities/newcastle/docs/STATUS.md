...
**Last updated:** 20 August 2026, post-close (**#54 FIXED**: run accounting is now read from the final iteration's own events stream, each stuck event attributed to the person's open leg; the engines' duplicate end-of-day aborts are counted separately — per-mode conservation closes on all three probes and on `bind1000_25pct`) (**every person-transport mode is now PHYSICALLY simulated, §9.49–§9.55**: freight PR #46; the §8.5 constrain-and-report decision PR #47 — its base arm owner-stopped at ~iteration 20, deliverable 5 still open; the four §9.51 owner directives PR #51 with the research dossiers and the measured gap decomposition; motorbike PR #52; physical ride boarding + walk/bike physical + the emergent ride share PR #53. **The demand family and the model changed several times today with no completed run between them — one family boundary, no comparison across it.** Active lane: **the owner's 4.5.0 sequencing decision, then the first converged run of the all-physical model** — every arm needs stated-cost approval) · the session landed on `main` via **PR #56** (the stacked PRs #46–#55 merged into their base branches; #56 carried the union). Start from `main`
...
| Blocking state | **None mechanical — the build side of every §9.51 directive that had a data anchor is DONE** (§9.52–§9.55: motorbike, physical boarding, physical walk/bike, emergent ride). **Active lane: the owner's 4.5.0 sequencing decision, then the FIRST CONVERGED RUN of the all-physical model** (~35 h at 25% × 1000, no standing approval) — it re-baselines everything at once: the emergent ride share vs 20.60, walk vs 13.40 (now carrying both #30's generation deficit AND §9.55's displaced ride mass), the §9.50 constrained base (C5 + report from its fit). #54's accounting fix has LANDED — `summarise_run.py` attributes stuck per mode from the events stream, so the run's own gate is trustworthy |
...
| Input registry | **327 fields** (§9.49 +7; §9.52 +3; §9.53 +1 `B.ride.physical_boarding`; §9.54 +7 net — walk/bike PCEs and speeds, road-rule exclusions, the declared access stub, `clearDefaultTeleportedModeParams`, MINUS the four retired teleported walk/bike fields; §9.55 +1 `B.ride.remode_unpaired`) — every one with units, provenance and a sweep, held-fixed rule or derived identity; ledger **0** with `--strict` gating CI |
