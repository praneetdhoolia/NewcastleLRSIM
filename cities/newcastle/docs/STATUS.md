...
**Last updated:** 20 August 2026 (**freight is PHYSICAL, §9.49, issue #24**: a `truck` mode in the mobsim at declared PCE — through-gate volumes split by each station's own observed heavy share, an internal tier over the observed freight-industry attractor, departures and weekend factors MEASURED from the classified hourly counts. Smoke-verified: 913 truck trips completed at 1%, 140,380 link traversals; the car fleet's physics unchanged, proven against the jar's bytecode. **A planned comparability break — `bind1000_25pct` is the last run of the §9.46/§9.47 family.** Earlier same day: §9.48 evaluated that arm — pairability 0.104% → 15.31% OD-coincident, occupancy 0.4855 vs 0.3503 outside range in the flattering direction, handed to 4.2.4. Active lane: **4.2.4/#14, the §8.5 calibration decision**) · branch `praneetdhoolia/freight-truck-mode`
...
| Blocking state | **None.** #5 CLOSED (§9.43). Ride pairing BUILT (§9.44), demand repair LANDED (§9.46/§9.47), re-measure arm EVALUATED (§9.48 — the ride lane rests). **Freight LANDED (§9.49, #24): `truck` is a qsim main mode at declared PCE, smoke-verified physical.** Active lane: **4.2.4/#14 — the §8.5 calibration decision and the calibrated base**, which must confront the §9.48 occupancy excess openly. Its runs need owner approval (state the cost, get a yes) |
...
| Input registry | **316 fields** (§9.49 added six `B.freight.*` and `RUN.qsim.car_vehicle`) — every one with units, provenance and a sweep, held-fixed rule or derived identity; ledger **0** with `--strict` gating CI; reach **74/74** |
...
| What is PHYSICALLY simulated (measured 20 Aug, §9.49) | **6 of 9 modes in the mobsim**: `car`; **`truck`** at declared PCE 2.0 (swept 1.5–3.5), capped at the regulated 100 km/h — smoke-verified 913 trips / 140,380 link traversals at 1%; **bus** 1,448 vehicles at PCE 2.8 sharing **22,102 road links with cars**; **rail** 332 on 6,766 dedicated links; **tram** 252, incl. **21 links shared on-street**; **ferry** 107. **`ride` is PAIRED but still teleported** (§9.44). Motorbike (no trip-share target, stays declined) and taxi/rideshare (4.4, deferred) are not modes |
...
| Runs on disk | `smoke_postrebuild` (plumbing, 1% × 2) · **`conv1000_10pct`** and **`conv1000_25pct`** (the #5 pilot arms, both rc=0, both **`relaxed: true`** — evaluation in [`docs/audit/CONVERGENCE_PILOT_EVALUATION.md`](audit/CONVERGENCE_PILOT_EVALUATION.md)). `results/_aborted_20260816/` quarantines three dead runs without `_run.json`, including the cancelled `conv1500_10pct` — **do not relaunch it**. ⚠️ `results/S2_WEEKDAY_f025_i1000_s20260810/` is a fourth dead run with no `_run.json`, not yet quarantined. `ride_pairing_probe` (3-iteration 1%, §9.44) and **`freight_smoke` (1% × 2, §9.49 — proves trucks physically move)** are PLUMBING PROBES, not results. `results/_aborted_20260818/bind1000_25pct` is the STOPPED first attempt at the re-measure arm. **`results/bind1000_25pct/` is the completed re-measure arm** — rc=0, `relaxed: true`, evaluated in §9.48 — and is now **the LAST run of the §9.46/§9.47 family: nothing run on the §9.49 freight demand compares to it** |
| Open issues | **5 → 4 at merge** — #24 (freight) is **DONE in this change (§9.49)** and closes when the PR lands, with its REOPEN IF condition. Remaining open: #9 (asc_car_passenger, belongs to 4.2.4) → #14 (calibrated base — **the active lane**); #28 (ride residual, open while ride stays teleported); #31 (ride constraint half + the realisation gap). **Closed on evidence 15–16 Aug:** #32, #36, #37 by the rebuild; #20, #29, #30, #34 after it — each closure comment states its REOPEN IF condition. Verdicts: [`docs/audit/ISSUE_VERDICTS.md`](audit/ISSUE_VERDICTS.md). |
