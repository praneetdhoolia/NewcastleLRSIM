...
*Generated by `src/calibrate/report.py` on 2026-08-20 from `_fit.json`. Regenerate it; do not edit it.*
...
|---|---|---|---:|---:|
| `bind1000_25pct` | S2 | WEEKDAY | 25% | 1000 |


## Mode share — the one block that carries the objective

**`bind1000_25pct`** — 5 targets, mean absolute error **6.45 pp**

| HTS category | modelled % | observed % | error (pp) |
|---|---:|---:|---:|
| Other | 3.92 | 3.20 | +0.72 |
| Public transport | 0.36 | 3.80 | -3.44 |
| Vehicle driver | 63.95 | 59.00 | +4.95 |
| Vehicle passenger | 31.05 | 20.60 | +10.45 |
| Walk only | 0.71 | 13.40 | -12.69 |
...
**`bind1000_25pct`** — 30 stations, light-vehicle basis

| statistic | value |
|---|---:|
| mean percentage error | -91.0% |
| mean absolute percentage error | 92.7% |
| RMSE | 27766 vehicles (113.4% of mean observed) |
| heavy-vehicle share assumed at | 27 of 30 stations |

**The model routes no traffic at all over 7 station(s):** `V078`, `V099`, `V103`, `V107`, `V109`, `V112`, `V170`. These are scored at −100%, not dropped: a modelled zero is a result, and the worst one in the set (issue #19). Dropping them flattered the fit by removing exactly where the model fails hardest.
...
- Vehicle occupancy: modelled **0.4855** passengers per driver against an observed **0.3503** (range [0.2493, 0.394]) — **OUTSIDE**

| mode | modelled km | observed km | ratio | in observed range |
|---|---:|---:|---:|---|
| bike | 9.03 | 5.20 | 1.74 | **no** |
| car | 10.40 | 10.20 | 1.02 | yes |
| pt | 11.78 | 23.40 | 0.50 | **no** |
| ride | 8.97 | 9.80 | 0.91 | yes |
| walk | 2.43 | 0.70 | 3.47 | **no** |

Ride-to-car trip length: modelled **0.862** against observed **0.961**. A ratio is robust to the geography mismatch that levels are not (§9.13).
...
**No calibration search has been run.** `C:\Users\Praneet Dhoolia\work\NewcastleLRSIM\cities\newcastle\params/C5_calibration.json` does not exist, so no parameter in this model has been moved by a calibration loop. Every value is its declared registry value, and the fit above is the fit of the *uncalibrated* base. This is stated rather than left to inference: P4 deliverable 5 is not met until that file exists.

## Where the fit is poor — stated plainly

On `bind1000_25pct`:

- **Vehicle passenger** is +10.45 pp out (modelled 31.05 against observed 20.60)
- **Walk only** is -12.69 pp out (modelled 0.71 against observed 13.40)
- **traffic counts** average -91.0% across 30 stations
- **7 station(s) carry no modelled traffic at all**
- **bike trip length** is 9.03 km against an observed 5.20 km
- **pt trip length** is 11.78 km against an observed 23.40 km
- **walk trip length** is 2.43 km against an observed 0.70 km
