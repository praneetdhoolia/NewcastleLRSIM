...
def build(run_dir):
    ...
    doc = {
        ...
        'not_a_result': (
            ...
            'Newcastle. It carries no mode share, no patronage, no fit '
...
def markdown(doc):
    ...
    A('> **This is not a result.** It records the state of the run, not a '
      'finding about Newcastle — no mode share, no patronage, no fit statistic, '
      'no comparison against a validation target. Those come from '
